...
> {
  ...
  async restoreWorkspace(workspaceId: string | number, userId: number) {
    ...
    // Validate admin manually because requireOwner middleware ignores deleted workspaces
    const member = await workspaceRepository.findMemberByUserId(id, userId);
    if (!member || member.role !== WorkspaceRole.ADMIN) {
      throw ApiError.forbidden(ErrorCode.FORBIDDEN_ACCESS, 'Only the admin can restore an archived workspace');
    }
